Attribute VB_Name = "QuaterlyStockSummary_Q4"
Sub QuarterlyStockSummary_Q4()
    ...
    For i = 2 To lastRow
        ...
        ' Process only rows within Q4 (October 1, 2022 to December 31, 2022)
        If ws.Cells(i, 2).Value >= DateValue("10/1/2022") And ws.Cells(i, 2).Value <= DateValue("12/31/2022") Then
            ' For the first Q4 row for a ticker, record the opening price
            If openingPrice = 0 Then
                openingPrice = ws.Cells(i, 3).Value  ' Column C: Open Price
            End If
            
            ' Update the closing price and accumulate volume (Columns F and G)
            closingPrice = ws.Cells(i, 6).Value      ' Column F: Close Price
    ...
    If outputRow > 2 Then
        ...
        ' Column P (16) header: "Ticker"
        ' Column Q (17) header: "Value"
        ws.Cells(1, 16).Value = "Ticker"
        ws.Cells(1, 17).Value = "Value"
        
        ' Output the additional results starting in row 2 (Column O: Metric, P: Ticker, Q: Value)
